<div class="-mt-2 relative" x-data="{ isOpen: true }">
    <div class="flex justify-between items-center h-20 -mt-7">
        ...
        <div x-data="{ show: true }">
            @if (session('message'))
                <x-alert title="{{ session('message') }}" positive class="bg-green-600 text-white" x-init="setTimeout(() => show = false, 5000)"
                    x-show="show" />
            @endif
        </div>
    </div>
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="me-5 h-[32px] flex justify-between items-center">
                    ...
                    <x-select shadow="false" placeholder="Seleziona Stato" wire:model.live="searchAvailable"
                        :options="[['label' => 'Approvato', 'value' => 1], ['label' => 'Da Approvare', 'value' => 0]]" option-label="label" option-value="value" />
                </div>

                <x-button icon="plus" black label="Aggiungi Progetto" class="font-bold w-[200px] h-[32px]"
                    wire:navigate href="/projects/create" />
            </div>
        </div>
