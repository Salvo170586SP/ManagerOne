<div class="-mt-2">
    <div class="flex justify-between items-center h-20 -mt-7">
        ...
        <div x-data="{ showError: true }">
            @if (session('error'))
                <x-alert title="{{ session('error') }}" negative class="bg-red-600 text-white" x-init="setTimeout(() => showError = false, 5000)"
                    x-show="showError" />
            @endif
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-300 rounded-lg {{-- h-[calc(100vh-13rem)] --}} overflow-y-auto p-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex justify-between items-center">
                ...
                @endif
                <x-button icon="plus" black label="Aggiungi Cliente" class="font-bold w-[200px] h-[32px]"
                    wire:navigate href="/clients/create" />
            </div>
        </div>
