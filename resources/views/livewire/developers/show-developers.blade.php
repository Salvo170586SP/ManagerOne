<div class="-mt-2">
    <div class="w-full flex justify-between items-center h-20 -mt-7">
        <h2 class="text-xl font-bold">Dettagli Developer</h2>
    ...
    <div class="flex flex-wrap w-full mx-auto text-black">
        <div class="w-full flex gap-3 mb-3">
            <div class="bg-white rounded-lg border border-gray-300 p-3 w-full  flex gap-3 items-center">
                <div class="flex items-center justify-center">
                    ...
                    @else
                    <div
                        class="w-[60px] h-[60px] border rounded-full bg-white overflow-hidden flex items-center justify-center">
            ...
            <div class="bg-white rounded-lg border border-gray-300 p-3 w-full flex flex-col justify-center">
                <div class="flex gap-2 items-center mb-3">
                    ...
                    <div class="text-xs whitespace-normal break-words">
                        {{ $developer->email ?? '-' }}
                    </div>
                </div>
                <div class="flex gap-2 items-center mb-3">
        ...
        <div class="w-full flex gap-3 mb-3">
            <div class="bg-white rounded-lg border border-gray-300 p-3 w-full flex flex-col justify-center">
                <div class="flex gap-2 items-center justify-between mb-2">
                    ...
                    @if ($developer->type)
                    <div
                        class="rounded-full min-w-[100px] max-w-[150px] text-center text-xs py-1 font-medium {{ $this->getColorType($developer->type) }}">
                ...
                <div class="flex gap-2 items-center justify-between mb-2">
                    ...
                    @if ($developer->level)
                    <div
                        class="rounded-full min-w-[100px] max-w-[150px] text-center py-1 text-xs font-medium {{ $this->getColorLevel($developer->level) }}">
            ...
            <div class="bg-white rounded-lg  border border-gray-300 p-3 w-full flex flex-col justify-center">
                ...
                <div class="flex gap-2 items-center justify-between">
                    ...
                    <div
                        class="rounded-full min-w-[100px] max-w-[150px] text-center text-xs py-1 font-medium {{ $this->getColorWorkplace($developer->workplace) }}">
                    ...
                    <tbody class="bg-white divide-y text-sm divide-gray-200">
                        ...
                        <tr wire:key="task-{{ $task->id }}-{{  str()->random(10) }}">
                            ...
                            <td class="px-6 py-4 whitespace-normal break-words max-w-[200px] text-center">
                                <x-button flat gray icon="eye" x-on:click="$openModal('tasks-{{ $task->id }}')" />
                            ...
                            <td class="px-6 py-4 whitespace-normal break-words max-w-[120px] text-center">
                                <x-button flat red icon="trash" x-on:click="$openModal('task-{{ $task->id }}')" />
                                <x-modal name="task-{{ $task->id }}" blur="sm" align="center">
                                    <x-card shadow="xl">
                                        <div
                                        ...
                                        <p class="text-base">
                                            Sei sicuro di eliminare definitivamente questa task?
                                        </p>

                                        <x-slot name="footer" class="flex justify-end gap-x-4 font-medium">
                                            <x-button black label="Annulla" x-on:click="close" />
                                            <x-button red label="Elimina" wire:click="deleteTask({{ $task->id }})" />
                                        </x-slot>
                                    </x-card>
                                </x-modal>
                            </td>
        ...
        <x-notes-sidebar wire:model="showDrawer2" :notes="$selectedTaskNotes" :item="$selectedTask"
