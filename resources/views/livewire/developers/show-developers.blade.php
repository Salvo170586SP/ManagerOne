    ...
    <div class="flex mx-auto text-black h-[calc(100vh-13rem)]">
        <div class="w-[350px] h-auto bg-white p-5 me-5 rounded">
            ...
            <div class="mb-4">
                ...
                @if($developer->type)
                <x-badge label="{{$developer->type}}"
                    class="rounded-xl {{$this->getColorType($developer->type)}}" />
                @endif
            </div>
            <div class="mb-4">
                <div class="font-medium text-sm">Grado In Azienda:</div>
                @if($developer->level)
                <x-badge label="{{$developer->level}}" class="rounded-xl {{$this->getColorLevel($developer->level)}}" />
                @endif
            </div>
            <div class="mb-4">
        ...
        <div class="w-full  p-6 bg-white rounded">
            ...
            <div>
                ...
                <div class="font-medium text-sm me-3">Team assegnato/i:</div>
                @foreach($developer->teams as $team)
                <span class="uppercase border px-2 py-1 rounded bg-gray-50 me-2">
                    {{$team->name}}
                </span>
                @endforeach
            </div>
