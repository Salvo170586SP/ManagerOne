<div class="-mt-2">
    ...
    <div class="bg-white rounded h-full h-[calc(100vh-13rem)] overflow-y-auto p-6">
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="me-5 h-[32px] flex justify-between items-center">
                    ...
                    <x-select shadow placeholder="Seleziona una città" wire:model.live="searchCity"
                        :options="$developers" option-label="city" option-value="city" />
                </div>
        ...
        <div class="flex gap-3">
            ...
            <x-card shadow="false" class="w-[350px] border my-5">
                <div class="flex justify-between">
                    <div class="bg-blue-700 w-[50px] h-[50px] flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="white" class="size-7">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Zm6-10.125a1.875 1.875 0 1 1-3.75 0 1.875 1.875 0 0 1 3.75 0Zm1.294 6.336a6.721 6.721 0 0 1-3.17.789 6.721 6.721 0 0 1-3.168-.789 3.376 3.376 0 0 1 6.338 0Z" />
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr wire:key="developer-{{$developer->id}}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        ...
                        <x-badge label="{{$developer->workplace}}"
                            class="rounded-xl {{$this->getColorWorkplace($developer->workplace)}}" />
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        @if($developer->level)
                        <x-badge label="{{$developer->level}}"
                            class="rounded-xl {{$this->getColorLevel($developer->level)}}" />
                        @endif
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        @if($developer->category)
                        <x-badge label="{{$developer->category}}"
                            class="rounded-xl {{$this->getColorCategory($developer->category)}}" />
                        @endif
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">{{$developer->createDate()}}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex justify-center">
                            <x-button flat black icon="eye" wire:navigate href="/developers/{{$developer->id}}" />
                            <x-button flat blue icon="pencil" wire:navigate
                                href="/developers/{{$developer->id}}/edit" />
                            <x-button flat red icon="trash" x-on:click="$openModal('simpleModal-{{$developer->id}}')" />
