<div class="mx-8 mt-4 hidden lg:block relative">
    <div class="bg-white border border-gray-300 rounded-lg w-full h-[80px] p-3 flex item-center justify-end"
        x-data="{ isOpen: false }"
        ...
        <div class="relative flex items-center" x-data="{ isOpen: false }" @click.away="isOpen = false">
            <x-button wire:click="markAsRead" @click="isOpen = !isOpen" icon="bell" lg black flat>
                @if ($unreadCount > 0)
                <span
                    class="absolute top-3 right-4 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full">{{
                    $unreadCount }}</span>
                @endif
            </x-button>
            <div x-show="isOpen" x-cloak x-transition:enter="transition ease-out duration-200"
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="inline-block w-6 h-6 rounded-full bg-gray-400 text-center text-white">{{
                            $notifications->count() }}</span>
                        <x-button wire:click="destroyAll" red flat label="Elimina tutte" icon="trash" class="text-sm" />
                    </div>
